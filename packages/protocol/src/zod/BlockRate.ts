...
export const BlockRateZod = z.object({
  ...
  rate: z.number().nonnegative().describe('number of milliseconds to make a block'),
  span: z.int().nonnegative().describe('number of blocks the rate was calculated over'),
  timeDifference: z.number().nonnegative().describe('time difference in milliseconds for start and end block'),
})
